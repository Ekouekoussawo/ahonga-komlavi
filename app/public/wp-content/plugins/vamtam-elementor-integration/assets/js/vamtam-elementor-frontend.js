...
( function( $, undefined ) {
	...
	var VAMTAM_ELEMENTOR = {
		...
		pageLoaded: function () {
			this.VamtamPopupsHandler.handleScrollBlockingPopup();
			// this.VamtamPopupToggleHandler.init();
		...
		VamtamPopupsHandler: {
			...
			handleScrollBlockingPopup() {
				const { classList } = document.body;

				if ( ! classList.contains( 'dialog-prevent-scroll' ) || ! classList.contains( 'vamtam-has-isbp' ) ) {
					return;
				}

				const popupClass = Array.from( classList ).find( ( cls ) => cls.startsWith( 'vamtam-isbp-' ) );

				if ( ! popupClass ) {
					return;
				}

				const popupID = popupClass.replace( 'vamtam-isbp-', '' );

				// Check if popup exists and clean up classes if it doesn't.
				if ( ! document.getElementById( 'elementor-popup-modal-' + popupID ) ) {
					classList.remove( 'dialog-prevent-scroll', 'vamtam-has-isbp', popupClass );
				}
			}
		},

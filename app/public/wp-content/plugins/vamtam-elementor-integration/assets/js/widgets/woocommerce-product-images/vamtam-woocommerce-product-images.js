class VamtamProductImages extends elementorModules.frontend.handlers.Base {
	...
	onInit( ...args ) {
		...
		this.groupElementorLightboxImages();
	}
	...
	groupElementorLightboxImages() {
		// Only for full-sized galleries
		const isFullSizedGallery = this.$element.hasClass( 'vamtam-has-full-sized-gallery' );
		if ( ! isFullSizedGallery ) {
			return;
		}

        // Check if Elementor global lightbox is active
        const elementorLightboxActive = elementorFrontend.getKitSettings( 'global_image_lightbox' );
        if ( ! elementorLightboxActive ) {
            return;
        }

        // If WC lightbox is active, we shouldn't interfere (handled by handleDoubleLightbox)
        const wcLightboxActive = jQuery( 'body' ).hasClass( 'wc-product-gallery-lightbox-active' );
        if ( wcLightboxActive ) {
            return;
        }

        // Create a unique slideshow ID based on this widget's ID
        const widgetId = this.getID();
        const slideshowId = 'vamtam_product_gallery_' + widgetId;

        // Find all image links and assign the slideshow ID
        // This tells Elementor that these images belong to the same slider
        const $links = this.$element.find( '.woocommerce-product-gallery__wrapper .woocommerce-product-gallery__image > a' );

        if ( $links.length ) {
            $links.attr( 'data-elementor-lightbox-slideshow', slideshowId );
        }
    }

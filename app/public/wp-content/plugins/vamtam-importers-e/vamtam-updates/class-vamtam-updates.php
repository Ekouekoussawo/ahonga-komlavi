...
class Vamtam_Updates_4 {
	...
	public function __construct( $file ) {
		...
		add_filter( 'admin_url', array( $this, 'plugin_admin_url_on_updates_page' ), 10 );
		...
		if ( isset( $upgrader->skin->plugin_info ) && false !== $upgrader->skin->plugin_info ) {
			...
			if ( $plugin_name === get_plugin_data( $this->full_path )[ 'Name' ] && empty( $package ) ) {
				return new WP_Error( 'no_package', sprintf(
					__( 'You need a valid Purchase Code to use the Automatic Updates feature. Register your Purchase Code <a href="%1$s">here</a> if you already have one, or <a href="%2$s" target="_blank">get one here</a>.', 'wpv' ),
					esc_url( admin_url( 'admin.php?page=vamtam_theme_setup' ) ),
					esc_url( 'https://vamtam.com/promo-envato-elements/?utm_source=plugin&utm_medium=update-notice&utm_campaign=update-placeholder' )
				) );
			}
		}

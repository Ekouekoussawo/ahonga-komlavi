...
class Vamtam_Import_Attachments extends WP_Background_Process {
	...
	protected function task( $task ) {
		...
		if ( is_array( $task ) && isset( $task['data'] ) ) {
			...
			$attachment_url       = $data['attachment'][1]; // Track the URL for marking as imported
			$imported             = $this->process_attachment( $data['attachment'][0], $attachment_url );
	...
	protected function process_attachment( $post, $url ) {
		...
		*/
		...
		if ( $post_exists && get_post_type( $post_exists ) === 'attachment' ) {
			$this->log( 'Skipping already-imported attachment: ' . $url . ' (existing post ID: ' . $post_exists . ')' );

			// Update the import map so ID remapping still works.
			$map = get_option( 'vamtam_last_import_map' );
			$map['posts'][ $post['import_id'] ] = $post_exists;
			update_option( 'vamtam_last_import_map', $map );

			// Build URL remap from the existing attachment.
			$existing_url = wp_get_attachment_url( $post_exists );
			$url_remap = [];
			if ( $existing_url ) {
				$url_remap[ $url ]            = $existing_url;
				$url_remap[ $post['guid'] ]   = $existing_url;
			}

			Vamtam_Importers_E::process_post_additional_data( $post, $post_exists, $post_exists, $map['authors'] );

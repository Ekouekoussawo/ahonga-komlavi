...
if ( vamtam_has_woocommerce() ) {
		...
		?>
			<div class="vamtam-empty-cart">
				...
				<div class="empty-cart-notice">
					<h5><?php esc_html_e( 'Your cart is currently empty.', 'caridad' ); ?></h5>
				</div>
			</div>
		<?php
	}
		...
		?>
			<?php if ( current_filter() === 'product_cat_edit_form_fields') : ?>
				<tr class="form-field">
					<th valign="top" scope="row">
						<label for="term_fields[class_level]"><?php echo apply_filters( 'vamtam_product_cat_class_level_label', esc_html__( 'Class Level', 'caridad' ) ); ?></label>
					</th>
					<td>
						<input type="text" size="40" value="<?php echo esc_attr( $class_level ); ?>" id="term_fields[class_level]" name="term_fields[class_level]"><br/>
						<span class="description"><?php echo apply_filters( 'vamtam_product_cat_class_level_description', esc_html__( 'e.g. Beginner, Advanced etc. ', 'caridad' ) ); ?></span>
					</td>
				</tr>
				<tr class="form-field">
					<th valign="top" scope="row">
						<label for="term_fields[class_category]"><?php echo apply_filters( 'vamtam_product_cat_class_category_label', esc_html__( 'Class Category', 'caridad' ) ); ?></label>
					</th>
					<td>
						...
						<span class="description"><?php echo apply_filters( 'vamtam_product_cat_class_category_description', esc_html__( 'e.g. A1, B2 etc.', 'caridad' ) ); ?></span>
					</td>
				</tr>
				<tr class="form-field">
					<th valign="top" scope="row">
						<label for="term_fields[class_description]"><?php echo apply_filters( 'vamtam_product_cat_class_description_label', esc_html__( 'Class Description', 'caridad' ) ); ?></label>
					</th>
					<td>
						...
						<span class="description"><?php echo apply_filters( 'vamtam_product_cat_class_description_description', esc_html__( 'Add a description for this class.', 'caridad' ) ); ?></span>
					</td>
				</tr>
			<?php elseif ( current_filter() === 'product_cat_add_form_fields' ) : ?>
				...
				<div class="form-field">
					...
					<span class="description"><?php echo apply_filters( 'vamtam_product_cat_class_description_description', esc_html__( 'e.g. A1, B2 etc.', 'caridad' ) ); ?></span>
				</div>
				<div class="form-field">
					<label for="term_fields[class_description]"><?php echo apply_filters( 'vamtam_product_cat_class_description_label', esc_html__( 'Class Description', 'caridad' ) ); ?></label>
	...
	function vamtam_woocommerce_after_subcategory_title( $term ) {
		...
		<?php if ( $show_btn ) : ?>
				<div class="vamtam-hover-wrap">
					<span class="vamtam-product-cat-btn button"><?php echo apply_filters( 'vamtam_template_loop_product_cat_button_text', esc_html__( 'See more', 'caridad' ) ); ?></span>
				</div>
			<?php endif; ?>
		<?php
	}
	...
	if ( VamtamElementorBridge::is_elementor_active() ) {
		...
		// Display product cats (after vamtam-product-content).
		function vamtam_display_product_cats() {
			global $product;
			// Hidden cats.
			if ( current_theme_supports( 'vamtam-elementor-widgets', 'products-base--hide-categories' ) ) {
				$hide_product_cats = isset( $GLOBALS['vamtam_wc_products_hide_categories'] ) && ! empty( $GLOBALS['vamtam_wc_products_hide_categories'] );
			...
			if ( ! \VamtamElementorBridge::is_build_with_elementor() ) {
				// Hide product cats on pages that are not built with Elementor.
				return;
			}
			echo '<div class="vamtam-product-cats">' . wc_get_product_category_list( $product->get_id(), '<span class="separator">  ・  </span>' ) . '</div>';
		}
		remove_action( 'woocommerce_after_shop_loop_item_title', 'vamtam_display_product_categories', 5 );
		add_action( 'vamtam_product_content_before_close', 'vamtam_display_product_cats', 101 );

		// Price after product-cats.
...
class Vamtam_WC_Ajax_Add_To_Cart_Handler {
	...
	private static function handle() {
		...
		if ( ! $is_valid && 0 === wc_notice_count( 'error' ) ) {
			if ( 'publish' !== $product_status ) {
				wc_add_notice( __( 'This product is not available for purchase yet.', 'caridad' ), 'error' );
			} else {
				wc_add_notice( __( 'Could not add this product to your cart. Please try again.', 'caridad' ), 'error' );
			}
		}
		...
		if ( $is_valid && $product_added ) {
			...
			$last_notice = end( $error_notices );
			$notice_text = ( is_array( $last_notice ) && ! empty( $last_notice['notice'] ) )
				? $last_notice['notice']
				: __( 'Could not add this product to your cart. Please try again.', 'caridad' );

			$data = array(
				'error' => true,
				'product_url' => apply_filters( 'woocommerce_cart_redirect_after_error', get_permalink( $product_id ), $product_id ),
				'notice' => '<div class="' . esc_attr( 'woocommerce-error vamtam-notice-error' ) . '" role="alert"><span class="vamtam-wc-msg">' . wc_kses_notice( $notice_text ) . '</span><span class="vamtam-close-notice-btn" /></div>',
			);
	...
	public static function handle_grouped_products() {
		...
		if ( ! empty( $items ) ) {
			...
			if ( ! $was_added_to_cart && ! $quantity_set ) {
				wc_add_notice( __( 'Please choose the quantity of items you wish to add to your cart.', 'caridad' ), 'error' );
	...
	public static function handle_variable_products() {
		...
		if ( empty( $variation_id ) && $product && $product->is_type( 'variable' ) ) {
			...
			wc_add_notice( sprintf( __( 'Please choose product options by visiting <a href="%1$s" title="%2$s">%2$s</a>.', 'caridad' ), esc_url( get_permalink( $product_id ) ), esc_html( $product->get_name() ) ), 'error' );

...
class VamtamOverrides {
	...
	public static function filters() {
		...
		add_action( 'give_post_form', [ __CLASS__, 'remove_give_form_footer' ], 1 );
	...
	/**
	 * Remove the navigation step counter and ssl/no-ssl notice
	 */
	public static function remove_give_form_footer() {
		echo '<style>.form-footer{opacity:0;height:0}</style>';
	}
	...
	public static function limit_wrapper() {
		...
		if ( VamtamElementorBridge::is_elementor_pro_active() && VamtamElementorBridge::is_build_with_elementor() ) {
			...
			$GLOBALS['vamtam_had_limit_wrapper'] = false;
		} else if ( vamtam_has_woocommerce() && is_shop() && VamtamElementorBridge::is_elementor_pro_active() ) {
			// Product archives are full width when Elementor Pro is active.
			$GLOBALS['vamtam_had_limit_wrapper'] = false;
		} else {
	...
	public static function oembed_dataparse( $output, $data, $url ) {
		// The .vamtam-video-frame wrapper is only for classic (non-block) front-end
		// output, where general.js resizes it. Skip it for block rendering and for
		// the block editor's oEmbed preview (served via the REST proxy) — that extra
		// wrapper has auto height and breaks core's sandbox aspect-ratio sizing,
		// leaving empty space below the iframe.
		if ( $data->type == 'video'
			&& ! ( has_blocks() && doing_filter( 'the_content' ) )

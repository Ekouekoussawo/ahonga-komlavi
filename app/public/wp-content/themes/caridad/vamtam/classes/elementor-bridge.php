...
class VamtamElementorBridge {
	...
	private function __construct() {
		...
		add_action( 'elementor/frontend/before_enqueue_scripts', [ __CLASS__, 'enqueue_frontend_scripts' ] );
	...
	public static function add_sticky_header_bg_color_to_elementor_global_settings() {
		...
		$kit->add_repeater_row( 'system_colors', [
			...
			'title' => __( 'Sticky Header Bg Color', 'caridad' ),
	...
	public static function enqueue_frontend_scripts() {
		...
		$use_min         = ! ( WP_DEBUG || ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) || ( defined( 'VAMTAM_SCRIPT_DEBUG' ) && VAMTAM_SCRIPT_DEBUG ) );
		$suffix          = $use_min ? '.min.js' : '.js';

		wp_enqueue_script(
			'vamtam-theme-elementor',
			VAMTAM_JS . 'theme-elementor' . $suffix,
			[
				'elementor-frontend',
			],
			$cache_timestamp,
	...
	*/
		...
		if ( $use_el_global_defaults ) {
			...
			for ( $i = 1; $i <= 8; $i++ ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( 'Accent %s', 'caridad' ), $i ),
	...
	protected static function add_theme_fonts_to_elementor_global_fonts() {
		...
		if ( $use_el_global_defaults ) {
			...
			// Adding from vamtam_get_option().
			$our_fonts = [
				'primary-font' => __( 'Primary Font', 'caridad' ),
				'h1'           => __( 'H1', 'caridad' ),
				'h2'           => __( 'H2', 'caridad' ),
				'h3'           => __( 'H3', 'caridad' ),
				'h4'           => __( 'H4', 'caridad' ),
			...
			foreach ( $our_fonts as $font_prefix => $font_title ) {
				...
				$kit->add_repeater_row( 'system_colors', [
					...
					'title' => sprintf( __( '%s', 'caridad' ), $font_title ),
	...
	public static function register_locations( $elementor_theme_manager ) {
		...
		$elementor_theme_manager->register_location(
			'page-title-location',
			[
				'label' => esc_html__( 'Page Title', 'caridad' ),
				'multiple' => true,
	...
	*/
	...
	public static function extract_zip( $file, $to ) {
		...
		if ( ! $unzip_result ) {
			$unzip_result = new \WP_Error( 'error', esc_html__( 'Could not unzip or empty archive.', 'caridad' ) );
		}

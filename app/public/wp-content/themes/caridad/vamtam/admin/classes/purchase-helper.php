...
class VamtamPurchaseHelper extends VamtamAjax {
	...
	private static function server_tests() {
		...
		$tests = array(
			array(
				'name'  => esc_html__( 'PHP Version', 'caridad' ),
				'test'  => version_compare( phpversion(), '5.5', '<' ),
				'value' => phpversion(),
				'desc'  => esc_html__( 'While this theme works with all PHP versions supported by WordPress Core, PHP versions 5.5 and older are no longer maintained by their developers. Consider switching your server to PHP 5.6 or newer.', 'caridad' ),
			),
			array(
				'name'  => esc_html__( 'PHP Time Limit', 'caridad' ),
				'test'  => $timeout > 0 && $timeout < 30,
				'value' => $timeout,
				'desc'  => esc_html__( 'The PHP time limit should be at least 30 seconds. Note that in some configurations your server (Apache/nginx) may have a separate time limit. Please consult with your hosting provider if you get a time out while importing the demo content.', 'caridad' ),
			),
			array(
				'name'  => esc_html__( 'PHP Memory Limit', 'caridad' ),
				'test'  => (int) $memory > 0 && $memoryB < 96 * 1024 * 1024,
				'value' => $memory,
				'desc'  => esc_html__( 'You need a minimum of 96MB memory to use the theme and the bundled plugins. For non-US English websites you need a minimum of 128MB in order to accomodate the translation features which are otherwise disabled.', 'caridad' ),
			),
			array(
				'name'  => esc_html__( 'PHP ZipArchive Extension', 'caridad' ),
				'test'  => ! class_exists( 'ZipArchive' ),
				'value' => '',
				'desc'  => esc_html__( 'ZipArchive is a requirement for importing the demo sliders.', 'caridad' ),
			),
		);
	...
	public static function tgmpa_plugins_complete_actions( $update_actions, $plugin_info ) {
		if ( isset( $update_actions['dashboard'] ) ) {
			$update_actions['dashboard'] = sprintf(
				esc_html__( 'All plugins installed and activated successfully. %1$s', 'caridad' ),
				'<a href="' . esc_url( admin_url( 'admin.php?page=vamtam_theme_setup_import_content' ) ) . '" class="button button-primary">' . esc_html__( 'Continue with theme setup.', 'caridad' ) . '</a>'
			);
	...
	public static function admin_menu() {
		add_menu_page( esc_html__( 'VamTam', 'caridad' ), esc_html__( 'VamTam', 'caridad' ), 'edit_theme_options', 'vamtam_theme_setup', array( __CLASS__, 'page' ), '', 2 );
		add_submenu_page( 'vamtam_theme_setup', esc_html__( 'Dashboard', 'caridad' ), esc_html__( 'Dashboard', 'caridad' ), 'edit_theme_options', 'vamtam_theme_setup', array( __CLASS__, 'page' ) );
	...
	public static function admin_menu_1() {
		...
		add_submenu_page( 'vamtam_theme_setup', esc_html__( 'Import Demo Content', 'caridad' ), esc_html__( 'Import Demo Content', 'caridad' ), 'edit_theme_options', 'vamtam_theme_setup_import_content', array( __CLASS__, 'vamtam_theme_setup_import_content' ) );
	}

	public static function admin_menu_2() {
		add_submenu_page(
			'vamtam_theme_setup',
			esc_html__( 'Services', 'caridad' ),
			esc_html__( 'Deals Up To', 'caridad' ) .
			'<span id="vamtam-premium-services">' . esc_html__( '70% Off', 'caridad' ) . '</span>',
			'edit_theme_options',
	...
	private static function should_show_offers_notice() {
		...
		if ( false !== strpos( (string) $screen->id, 'caridad' ) || $screen->is_block_editor() ) {
			return false;
		}
	...
	public static function registration_warning() {
		...
		<div class="vamtam-notice-wrap">
			<div class="vamtam-notice">
				<p>
					<?php echo esc_html__( 'Please activate your license to get theme updates, premium support, and access to demo content.', 'caridad' ); ?>
	...
	public static function admin_init() {
		...
		$registration_label = esc_html__( 'Enter your purchase code from vamtam.com to receive theme updates and support.', 'caridad' );
		...
		<div id="vamtam-ts-homepage" class="vamtam-ts">
			...
			<div id="vamtam-ts-main">
								...
								<?php if ( $valid_key ) : ?>
									<div id="vamtam-verified-code">
										<p>
											...
											?>
											<br />
											<?php echo esc_html( sprintf( __( 'You can now enjoy %s and build great websites.', 'caridad' ) , $theme_name ) ); ?>
										</p>
									</div>
								<?php else : ?>
									<div id="vamtam-envato-market-radios">
										<div>
											<label>
												...
												<span><?php echo esc_html__( 'vamtam.com', 'caridad' ); ?></span>
											</label>
											<label>
												<input type="radio" id="vamtam-envato-market-radio" name="vamtam_envato_elements">
												<span><?php echo esc_html__( 'Envato Market', 'caridad' ); ?></span>
											</label>
											<label>
												<input type="radio" id="vamtam-envato-elements-radio" name="vamtam_envato_elements">
												<span><?php echo esc_html__( 'Envato Elements', 'caridad' ); ?></span>
											</label>
										</div>
									</div>
				...
				<div id="vamtam-check-license-disclaimer">
					<h5><?php esc_html_e( 'Licensing Terms', 'caridad' ); ?></h5>
					<p>
						<?php
						...
						foreach ( $terms as $type => $t ) {
							$terms_links .= '<a class="vamtam-licensing-terms-link" data-license-type="' . esc_attr( $type ) . '"'
								...
								. sprintf( esc_html__( '%s terms here', 'caridad' ), esc_html( $t['label'] ) )
								. '</a>';
						}
						printf(
							/* translators: %s: link to the licensing terms for the selected license type, e.g. "Envato Market terms here". */
							esc_html__( 'You need to register a separate license for each domain on which you will use the theme. A single license is limited to a single domain/application. For more information, please refer to the %s.', 'caridad' ),
						...
						<div id="vamtam-server-tests">
							<h3>
								<?php if ( $status['fail'] > 0 ) : ?>
									<?php esc_html_e( 'System Status', 'caridad' ) ?>
									<?php $fail = $status['fail']; ?>
									<small><?php printf( esc_html( _n( '(%d potential issue)', '(%d potential issues)', $fail, 'caridad' ) ), $fail ) ?></small>
								<?php endif ?>
							</h3>
						</div>
					<?php endif ?>
	...
	public static function dashboard_navigation() {
		...
		<nav id="vamtam-ts-nav-menu">
			<div id="vamtam-theme-title">
				<span id="vamtam-ts-greeter"><?php esc_html_e( 'WELCOME TO', 'caridad' ); ?></span>
				<span id="vamtam-ts-greeter-title"><?php echo esc_html( $theme_name ); ?></span>
				<span id="vamtam-ts-greeter-ver"><?php echo sprintf( esc_html__( 'VER. %s', 'caridad' ), $theme_version ); ?></span>
			</div>
			<ul>
				<li class="<?php echo esc_attr( $cur_route === 'toplevel_page_' . $routes[0] ? 'is-active' : '' ); ?>" >
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[0] ) ); ?>">
						...
						<span><?php echo esc_html__( 'Register' , 'caridad' ); ?></span>
				...
				<?php if ( isset( $tgmpa_instance ) && isset( $tgmpa_instance->page_hook ) ) : ?>
					<li class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[1] ? 'is-active' : '' ); ?>" >
						<a <?php echo esc_attr( ! $valid_key ? 'class=disabled' : '' ); ?> href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[1] ) ); ?>">
							...
							<span><?php echo esc_html__( 'Install Plugins' , 'caridad' ); ?></span>
				...
				<li class="<?php echo esc_attr( $cur_route === 'vamtam_page_' . $routes[2] ? 'is-active' : '' ); ?>" >
					<a <?php echo esc_attr( ! $valid_key || $plugin_status !== 'success' ? 'class=disabled' : '' ); ?> href="<?php echo esc_url( admin_url( 'admin.php?page=' . $routes[2] ) ); ?>">
						...
						<span><?php echo esc_html__( 'Import Demo' , 'caridad' ); ?></span>
	...
	public static function import_buttons() {
		...
		$buttons = array(
			array(
				'label'          => esc_html__( 'Dummy Content Import', 'caridad' ),
				'id'             => 'content-import-button',
				'description'    => esc_html__( 'You are advised to use this importer only on new WordPress sites.', 'caridad' ),
				'button_title'   => $content_imported ? esc_html__( 'Imported', 'caridad' ) : esc_html__( 'Import', 'caridad' ),
				...
				'data'           => array_merge( $messages, [
					...
					'success-msg'      => sprintf( esc_html__( 'Main content imported. Image import progress: <span class="vamtam-image-import-progress">%s</span>.', 'caridad' ), $img_progress ),
					'fail-msg'         => esc_html__( 'Failed to import. We recommend that you contact your hosting provider for advice, as solving this issue is often specific to each server.', 'caridad' ),
					'timeout-msg'      => esc_html__( 'Failed to import. This is most likely caused by a timeout. Please contact your hosting provider for advice as to how you can increase the time limit on your server.', 'caridad' ),
				] ),
				'additional_msg' => $import_disabled_msg . wp_kses( sprintf( __( '<p class="vamtam-description">Please make sure to <a href="%s" target="_blank">backup</a> any existing content that you need as it will be removed by the import procedure (affects Posts, Pages and Menus).</p><p class="vamtam-description">We recommend that you use the <a href="%s" target="_blank">Post Name permalink structure</a></p><p class="vamtam-description">Images will be downloaded in the background after the main import is complete. Depending on your server, this may take several minutes.<br> In the meantime you may notice that some images are not visible.', 'caridad' ), esc_url( admin_url( 'plugin-install.php?tab=plugin-information&plugin=updraftplus&TB_iframe=true&width=772&height=921' ) ), esc_url( admin_url( 'options-permalink.php' ) ) ), 'vamtam-admin' ),
				'disabled_msg_plain' => '',
			),
		);
	...
	public static function purchase_key( $args ) {
		...
		$placeholder = __( 'XXXXXX-XXX-XXXX-XXXX-XXXXXXXX', 'caridad' );
		$plugin_status = VamtamPluginManager::get_required_plugins_status();
		$content_imported = ! ! get_option( 'vamtam_last_import_map', false );
		...
		if ( $valid_key ) {
			...
			esc_html_e( 'Valid', 'caridad' );
			echo '</span>';
		} else {
			$invalid_badge = '<span class="invalid"><span class="dashicons dashicons-no-alt"></span>' . esc_html__( 'Invalid', 'caridad' ) . '</span>';
		...
		if ( ! defined( 'ENVATO_HOSTED_SITE' ) ) {
			...
			if ( ! $valid_key ) {
				...
				echo wp_kses( sprintf( __( ' <a href="%s" target="_blank">Where can I find my Item Purchase Code?</a>', 'caridad' ), 'https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-' ), 'vamtam-a-span' );
				echo wp_kses( sprintf( __( ' <a href="%s" target="_blank">Cannot validate Purchase Code?</a>', 'caridad' ), 'https://elementor.support.vamtam.com/support/solutions/articles/252289-cannot-validate-purchase-key-or-token' ), 'vamtam-a-span' );
				echo '</span>';
				echo '<span id="vamtam-help-own-store">';
				echo wp_kses( sprintf( __( 'Your vamtam.com purchase code is available in your <a href="%s" target="_blank">Order Details</a>', 'caridad' ), 'https://vamtam.com/my-account/orders/' ), 'vamtam-a-span' );
				echo wp_kses( sprintf( __( ' <a href="%s" target="_blank">Cannot validate Purchase Code?</a>', 'caridad' ), 'https://elementor.support.vamtam.com/support/solutions/articles/252289-cannot-validate-purchase-key-or-token' ), 'vamtam-a-span' );
				echo '</span>';
				echo '</p>';

				echo '<p id="vamtam-code-help-elements" class="hidden">';
				echo wp_kses( sprintf( __( ' <a href="%s" target="_blank">Where to get the License Code from</a>', 'caridad' ), 'https://elementor.support.vamtam.com/support/solutions/articles/252289-cannot-validate-purchase-key-or-token' ), 'vamtam-a-span' );
				echo '</p>';
				...
				echo '>' . esc_html__( 'Register', 'caridad' );
				echo '</button>';
				...
				echo esc_html__( 'Continue to required plguins', 'caridad' );
				echo '</a>';
				...
				echo esc_html__( 'Continue to demo import', 'caridad' );
				echo '</a>';
			}
